...
# 18. Inferring a Continuous-Time System from Discrete-Time Data: An Appreciation of A. W. Phillips (1959)
...
written by Lars Peter Hansen and one of us.[^fn18-appreciation]
```
...
spectral density matrix**[^fn18-rational] — exactly the class of processes that Chapter 8
...
{doc}`20_dimensionality_aliasing_problem`. What follows reconstructs the steps by which
...
rewrites the convolution system {eq}`eq-18-sys` as a system of *differential equations* and
...
exactly what a numerator of equal or higher degree would produce in the inverse transform.[^fn18-delta]
...
Equation {eq}`eq-18-theta` deserves to be recognized for what it is: it is the **spectral
...
Combining {eq}`eq-18-Q` and {eq}`eq-18-R`, the whole system reads $Q(D)x(t) = R(D)\zeta(t)$.
...
Repeating the filtering calculation that produced {eq}`eq-18-theta`, the spectral density
...
are distinct with negative real parts, he expands {eq}`eq-18-Phi` in partial fractions and
inverts. The unique inverse compatible with stationarity — the one decaying as $\tau \to
\pm\infty$ — is

```{math}
:label: eq-18-phi-cont
...
{eq}`eq-18-phi-cont` is the centrepiece, and it is precisely the result (b) promised at the
...
function $M(z) = \sum_{\tau \ge 0} \phi_\tau z^\tau$. Substituting {eq}`eq-18-phi-cont` and
...
Equation {eq}`eq-18-log` is where the **aliasing problem** is born, and it is the same
phenomenon as the *folding formula* of Chapter 15. The complex logarithm is multivalued: if
...
the observable band, exactly as Chapter 15's $S^d(\omega) = \sum_n S(\omega - 2\pi n)$ folds the
...
the project of {doc}`20_dimensionality_aliasing_problem`, where the requirement that the implied
...
factors $\mu_r$, and {eq}`eq-18-log` returns the continuous poles $\lambda_r$. The whole
...
{doc}`20_dimensionality_aliasing_problem` and the rational-expectations literature later used to
overcome aliasing.
...
({doc}`16_time_aggregation_var`, {doc}`19_aggregation_inverse_optimal_predictor`).
...
{eq}`eq-18-log`: P. C. B. Phillips (1972, 1973, 1974) and Hansen and Sargent (1983, 1991)
characterized exactly when, and how nearly, a continuous-time model can be recovered from
skip-sampled data — the analysis of {doc}`20_dimensionality_aliasing_problem`. The second,
pursued by Christopher Sims (1971) and John Geweke (1978), asks how closely discrete-time
distributed lags resemble continuous-time ones; Sims's formula expresses the discrete lag as a
...
({doc}`21_temporal_aggregation_streamlined`).
...
works.[^fn18-wheels] More recently, Hansen and Scheinkman (1995) extended the questions Phillips
...
**Aliasing and the multivalued logarithm.** Phillips's map {eq}`eq-18-log`,
...
of $2\pi$ — i.e., only up to the Nyquist frequency $\pi$ of unit-interval sampling (Chapter 15).

```{exercise-end}
```

```{solution-start} ex-18-1
:class: dropdown
```
...
Chapter 15, quantified in {doc}`20_dimensionality_aliasing_problem`.
...
(a one-term version of {eq}`eq-18-phi-cont` with pole $\lambda = -a$).

(a) Sample $\phi$ at the integers and form Phillips's generating function $M(z) = \sum_{\tau \ge 0}
\phi_\tau z^\tau$. Verify it equals $\phi_0/(1 - \mu z)$ with $\mu = e^{-a}$, as {eq}`eq-18-M`
predicts.

(b) Recover the continuous-time parameters from the sampled autocovariances alone:
$\mu = \phi_1/\phi_0$, then $a = -\log\mu$ from {eq}`eq-18-log`, and $b^2 = 2a\,\phi_0$.

(c) Exhibit the discrete-time shadow {eq}`eq-18-discrete`: the sampled process is the AR(1)
...
{eq}`eq-18-discrete` in one dimension. (Because the pole $\lambda = -a$ is *real*, there is no
...
(a) Sample at the integers and verify Phillips's identity {eq}`eq-18-M`: the generating function
...
(b) Recover the continuous poles from the discrete decay factors via {eq}`eq-18-log`,
$\lambda_r = \log\mu_r$, and confirm $\lambda_r = -a_r$.

```{exercise-end}
```

```{solution-start} ex-18-3
:class: dropdown
```
...
The discrete generating function reproduces Phillips's partial-fraction form {eq}`eq-18-M`
...
[^fn18-appreciation]: The framing of this chapter, the list of consequences (a)–(g) of a
...
[^fn18-rational]: For a continuous-time process, a rational spectral density is one expressible
...
[^fn18-delta]: If a numerator $\beta_{ij}(s)$ were of degree equal to or greater than its
...
[^fn18-wheels]: A manifestation of aliasing familiar to movie-goers is the illusion that a
