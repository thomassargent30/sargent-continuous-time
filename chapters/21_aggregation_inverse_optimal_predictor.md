...
the 1983 original (flagged in {ref}`sec-21-5`).
```
...
forecast in {doc}`16_nonstationary_examples` (equation {eq}`eq-16-2` there); this chapter takes
...
To solve stochastic differential equation {eq}`eq-21-1`, Hansen and Sargent write it shifted
...
realizable, the authors assume that anticipated inflation is. Equation {eq}`eq-21-5` also
...
where $\delta$ is the Dirac delta generalized function. They assume that {eq}`eq-21-6` holds for
some $t$ greater than or equal to a start-up time $T$, with $w(t) = 0$ for $t < T$. That
{eq}`eq-21-6` is a Wold representation means that instantaneous forecast errors in predicting an
element of $x(t)$ from past $x$'s are linear combinations of the elements of $w(t)$.

Hansen and Sargent write the first row of {eq}`eq-21-6` as
...
Shifting {eq}`eq-21-7` forward $v$ time units and taking expectations, they find
...
equation {eq}`eq-21-1` empirical content, something must be said about the dynamic correlation
...
for some scalar constant $k_2$. Combining restrictions {eq}`eq-21-14` and {eq}`eq-21-12`
determines that
...
For the remainder of the analysis, Hansen and Sargent adopt {eq}`eq-21-17` as a simplifying
...
In the continuous-time system {eq}`eq-21-19`, $m$ fails to Granger cause $p$ because
$c_{12}(D) = 0$, while $p$ does Granger cause $m$. That these features characterize the system
is no surprise: {eq}`eq-21-19` was constructed precisely to guarantee that Cagan's adaptive
expectations mechanism {eq}`eq-21-5` is optimal. In light of {eq}`eq-21-4`, if Cagan's mechanism
is rational there must be extensive feedback from $p$ to $m$.[^fn21-1]
...
Then {eq}`eq-21-22` can be written
...
process. From {eq}`eq-21-24`, $(1-L)^2 p$ is a first-order moving average with covariance
generating function
...
integers. Representation {eq}`eq-21-26` can then be written
...
Letting $X_t = p(t) - p(t-1)$, $M_t = m(t) - m(t-1)$, one can write {eq}`eq-21-32` as
...
rational, given {eq}`eq-21-37`. In form, {eq}`eq-21-37` matches {eq}`eq-21-34`. A central task is
now to study the relation between the $(2 \times 2)$ matrix $F$ in {eq}`eq-21-34` and the
