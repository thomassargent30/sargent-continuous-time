```{eval-rst}
.. index::
   single: time aggregation; and vector autoregressions
   single: vector autoregression; and time aggregation
   single: Granger causality; and time aggregation
   single: innovation; discrete time versus continuous time
   single: Wold representation; discrete time
   single: point-in-time sampling
...
$p(\tau)$. Sometimes we shall find it convenient to write {eq}`eq-18-2-1` in operator notation
...
to specifying that {eq}`eq-18-2-1` is a Wold representation for $z(t)$.
...
to the side condition that the zeroes of $\det C(z)$ do not lie inside the unit circle:[^fn18-1]
...
The right side of {eq}`eq-18-2-4` is the *folding* of the continuous time spectral density onto
the discrete-frequency band. It is the matrix counterpart of the folding formula
{eq}`eq-17-fold` of {doc}`17_discrete_sampling_folding`, and the source of the aliasing that
the rest of this chapter traces.
...
$z$. It follows directly upon writing out {eq}`eq-18-2-5` that
...
It also follows from {eq}`eq-18-2-6` and the identity for integer $t$, $C(L) a_t = P(D) w_t$, that
...
Equations {eq}`eq-18-2-6` and {eq}`eq-18-2-7` show how the discrete time innovation $a_t$ in
...
so that in {eq}`eq-18-2-6`, $f(\tau) = 0$ for $\tau > 1$. In that case, $a_t$ would be a weighted
sum of the continuous time innovations over the unit forecast interval. It would be even more
desirable if {eq}`eq-18-2-9` were to hold with $f(\tau) = p(\tau)$, for then $a_t$ would equal the
...
This last equality implies that if {eq}`eq-18-2-9` is to hold it must be the case that
...
requirement {eq}`eq-18-2-10` is that the discrete time and continuous time forecasts of $z(t)$ over
a unit time interval coincide.

When condition {eq}`eq-18-2-9` is met, the link between $P(D)$ and $C(L)$ is particularly simple.
Using $f(\tau) = 0$ for $\tau > 1$, equation {eq}`eq-18-2-8` becomes
...
Equation {eq}`eq-18-2-11` implies that for the particular class of continuous time processes for
...
are observationally equivalent. Relation {eq}`eq-18-2-11` also implies that in general, without
some more restrictions on $p(\tau)$, condition {eq}`eq-18-2-9` does not place *any* restrictions on
...
the continuous time moving-average coefficients be a continuous function of $\tau$.[^fn18-3] This
requirement together with {eq}`eq-18-2-11` then imposes a very stringent restriction on the
discrete time moving-average representation. In particular, {eq}`eq-18-2-11` then implies that
...
same dimension ($m = n$) and $f(0)$ is nonsingular, relation {eq}`eq-18-2-12` implies that
...
This implies that if {eq}`eq-18-2-9` is to hold, the discrete time process must have a first order
autoregressive representation. We have therefore established that condition {eq}`eq-18-2-9` and the
...
Thus, with a continuous $p(\tau)$ function, in general, relation {eq}`eq-18-2-9` does not hold.
Instead, $a_t$ given by {eq}`eq-18-2-6` is a function of all current and past $w(t)$'s, a function
...
Second, the matrix function $f(\tau)$ in {eq}`eq-18-2-6` is not usually diagonal, so that each
...
function $f(\tau)$ in {eq}`eq-18-2-5` in general is nonzero for all values of $\tau > 0$, so that
...
the class introduced in {doc}`08_spectral_densities` and generated by the linear stochastic
differential equations of {doc}`11_linear_sde`, whose sum-of-exponentials kernel
$p(\tau) = \sum_j \delta_j e^{\lambda_j \tau}$ reappears below.
Thus we assume that
...
To find this representation, we must use {eq}`eq-18-2-4`. A.W. Phillips (1959) and Hansen and
Sargent (1983b) show that for the process {eq}`eq-18-2-13`, the term on the right side of
{eq}`eq-18-2-4` can be represented
...
term on the right side of {eq}`eq-18-2-18` can be expressed as
...
Recalling that $\alpha_j = e^{\lambda_j}$, equation {eq}`eq-18-2-22` implies that
...
Equations {eq}`eq-18-2-17` and {eq}`eq-18-2-24` imply that $C_k$ will be (proportional to) a sampled
version of $p(\tau)$ if and only if $\gamma_j/\delta_j = \gamma_1/\delta_1$ for all $j = 2, \ldots, r$.
It can be shown directly by using {eq}`eq-18-2-17` and {eq}`eq-18-2-24` in {eq}`eq-18-2-7` and
...
Figure 6. The aggregation kernel $f(\tau)$ of {eq}`eq-18-2-7` for the example of Table 1. On $[0,1]$ it coincides with $p(\tau)$; it does not vanish for $\tau > 1$, and is in fact larger in absolute value over most of $[1,2]$ than over $[0,1]$, a consequence of the system being third order rather than first order in continuous time.
```
...
$z_t$. Matrix versions of {eq}`eq-18-2-17` and {eq}`eq-18-2-24` hold, where the $\lambda_j$'s are
the zeroes of $\det \theta(s)$ and the $\delta_j$'s and the $\gamma_j$'s are $(n \times n)$ matrices
given by {eq}`eq-18-2-16` and {eq}`eq-18-2-23`.
...
```{eval-rst}
.. index::
   single: locally unpredictable process; and vector autoregressions
   single: linear quadratic economy; and local unpredictability
   single: adjustment costs; and mean square differentiability
   single: forecast error; sampled
   single: sampling interval; shrinking
```

## (a) Locally Unpredictable Processes and Linear Quadratic Models

```{note}
This section reproves, in the notation of the original paper, results that earlier chapters
...
{doc}`13_locally_unpredictable`. Its computation $g(0) = \lim_{s\to\infty} sG(s) = P(-\rho)$ is
the calculation performed there. The citations to "Sargent (1983)" in the text below are the
original paper's; within this book they point to Chapters 9 and 13. Two things here are new.
Equation {eq}`eq-18-2-26` bridges local unpredictability and the behaviour of the sampled
forecast error as the sampling interval shrinks. And adjustment-cost models smooth $x^*(t)$ back
into mean square differentiability, so appealing to that limit does not escape the difficulty.
```
...
$\{x(t-s),\ s \geq 0\}$. Now condition {eq}`eq-18-2-25` can readily be shown to imply that
...
In {eq}`eq-18-2-26`, $\hat{E}_t x(t+\delta)$ is the linear least squares projection of $x(t+\delta)$
...
{eq}`eq-18-2-26` holds for any locally unpredictable process, and states that for small enough
...
$a_t$ in the $\delta$-counterpart to {eq}`eq-18-2-21` is arbitrarily close to
$\int_0^{\delta} p(s)w(t-s)\, ds$ in the mean square sense.

Now suppose that $z(t)$ is given by {eq}`eq-18-2-1`, with $p(0) = 0$, so that $z(t)$ is mean square
...
Using the initial value theorem together with {eq}`eq-18-2-28`, we find that
...
Here $g$, not $p$, is the relevant kernel, since it is $x^*$ rather than $z$ that is being
...
like $x^*(t)$ and are governed by a version of {eq}`eq-18-2-27`. For example, with $x^*(t)$ being
consumption and $z(t)$ income, {eq}`eq-18-2-27` is a version of the permanent income theory.
Alternatively, with $x^*(t)$ being a stock price and $z(t)$ being the dividend process,
{eq}`eq-18-2-27` is a simple version of an asset-pricing formula.
...
based on the limit {eq}`eq-18-2-26`.

```{eval-rst}
.. index::
   single: cross-equation restrictions
   single: continuous time model; as the primitive object
   single: identification; through structural restrictions
```
...
.. index::
   ...
   single: Hansen, L. P.
   single: Sargent, T. J.
   single: permanent income theory
   single: consumption; and income
```
...
[^fn18-1]: Practical methods for solving this equation for the case in which $P(s)$ is rational are
    discussed by Phillips (1959), Hansen and Sargent (1980b), and Christiano (1980).

[^fn18-2]: An alternative derivation of {eq}`eq-18-2-7` uses operational calculus. Setting
    $L = e^{-D}$, express {eq}`eq-18-2-5` as $a_t = V(e^{-D}) P(D) w(t) \equiv f(D) w(t)$. Here the
    function $f(\tau)$ is the inverse Fourier transform of $F(i\omega)$, which is defined by
    $$F(i\omega) = C(e^{-i\omega})^{-1} P(i\omega).$$
    Equation {eq}`eq-18-2-7` follows from the above equation by the convolution property of Fourier
    transforms.

[^fn18-3]: For example, the function $p(\tau)$ will be continuous whenever $P(D)$ is rational, a
...
[^fn18-4]: See Sargent (1983) for definitions of mean square continuity and mean square
    differentiability.
```
...
```{note}
The citations in the text above are the original paper's, and they carry that paper's letter
suffixes. Hansen and Sargent published several closely related papers in 1980, 1981, and 1983,
and the entries below record the ones the suffixes draw on. Two of the works cited appear in
this book: the geometric distributed lead {eq}`eq-18-2-28` attributed to Hansen and Sargent
(1981d) is derived in {doc}`19_prediction_formulas_continuous_time`, and is the formula
{eq}`eq-12-gen` of {doc}`12_prediction`; the discrete time representation attributed to Hansen
and Sargent (1983b) is the subject of {doc}`22_dimensionality_aliasing_problem`. The citations
to "Sargent (1983)" are to the 1983 class notes that became Chapters 1 through 14 and 17 of
this book; for mean square continuity and differentiability see
{doc}`02_mean_square_continuity_differentiability` and
{doc}`09_characterizations_ms_differentiability`.
```
...
Models from Discrete Time Data. Unpublished manuscript. Later published in L. P. Hansen and
T. J. Sargent, eds., *Rational Expectations Econometrics*. Boulder: Westview Press, 1991.

Hansen, L. P., and T. J. Sargent (1982). Continuous Time Models of Control, Prediction, and
Rational Expectations.

Hansen, L. P., and T. J. Sargent (1983). Aggregation Over Time and the Inverse Optimal Predictor
Problem for Adaptive Expectations in Continuous Time. *International Economic Review*, **24**,
1–20. (Reported in {doc}`20_aggregation_inverse_optimal_predictor`.)

Hansen, L. P., and T. J. Sargent (1983). The Dimensionality of the Aliasing Problem in Models
with Rational Spectral Densities. *Econometrica*, **51**, 377–387. (Reported in
{doc}`22_dimensionality_aliasing_problem`.)
...
Equations. *Biometrika*, **46**, 67–76. (Discussed in
{doc}`21_phillips_continuous_time_estimation`.)

Sargent, T. J. (1983). Continuous Time Stochastic Processes. Class notes, University of
Minnesota. (Chapters 1 through 14 and 17 of this book.)

Sims, C. A. (1984). Martingale-Like Behavior of Prices and Interest Rates. Discussion paper,
