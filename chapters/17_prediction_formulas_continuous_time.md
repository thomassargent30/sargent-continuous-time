# 17. Prediction Formulas for Continuous Time Linear Rational Expectations Models
...
geometric distributed leads did in Hansen and Sargent (1980a).
...
The stochastic integral in {eq}`eq-17-1-3` can be interpreted as the limit point of a
mean-square convergent sequence of random variables (e.g. see Rozanov 1967). Relation
{eq}`eq-17-1-3` gives a convenient mapping between the space $L^2$ of functions and the space
...
{eq}`eq-17-1-1` and {eq}`eq-17-1-2`, is
...
where $x_1$ and $x_2$ are given by convolution {eq}`eq-17-1-3` using $\phi_1$ and $\phi_2$
respectively.
...
variables $x(t)$ given by {eq}`eq-17-1-3` for $\phi$'s restricted to be in $L^2_+$. It follows
from {eq}`eq-17-1-4` that since $L^2_+$ is a Hilbert space, so is $H(t)$. Furthermore, the
...
$\phi^+$ and $dW$ for $\phi^+$ given in {eq}`eq-17-1-5`.

## 2. Transforms

One convenient way to represent functions in $L^2$ and characterize mapping {eq}`eq-17-1-5`
...
extension, the spectral density function for $x$ generated via {eq}`eq-17-1-3` is just
...
of the form {eq}`eq-17-1-6`, we use Laplace transforms. These transforms are defined as
...
$\phi^+$ is defined in {eq}`eq-17-1-5`? To answer this question, we first study Laplace
...
right side of {eq}`eq-17-2-3` gives a uniform bound (in $\mathbf{c}$) on $\mathcal{L}p(\phi)$
...
Formula {eq}`eq-17-3-4` is the continuous time counterpart to formula (5) in Hansen and Sargent
(1980a).
...
have the following generalization of {eq}`eq-17-3-4`:
...
Suppose that $\gamma$ is given by {eq}`eq-17-3-3`, and $\mathcal{L}p(\psi)$ is a rational
function:
...
The right side of {eq}`eq-17-3-5` has a removable singularity at $\delta$ by construction. This
...
By equating coefficients of the polynomials on both sides of {eq}`eq-17-3-6`, one can construct
...
increments. We now replace {eq}`eq-17-1-1` and {eq}`eq-17-1-2` with
...
Laplace transforms of entries in $\phi$. In Example 1 formula {eq}`eq-17-3-4` still applies where
...
process as starting at some initial time, say $t = 0$. Hence we imagine {eq}`eq-17-1-1` and
{eq}`eq-17-1-2` holding for nonnegative values of $t_1$, $t_2$, $t_3$ and $t_4$, and we assume
that the random measure of any interval contained in $(-\infty, 0)$ is zero. This permits
formula {eq}`eq-17-3-1` to be well defined for a much larger class of functions $\psi$. We might
...
positive values of $\rho$ which allows for polynomial growth in the second moment of $y$.[^fn17-3]
...
[^fn17-1]: The uniqueness of this decomposition requires some qualification. Elements of $L^2$
...
[^fn17-2]: We take the right side of equation {eq}`eq-17-3-1` as the definition of $x(t)$.
    Alternatively, for particular classes of $\gamma$ we could define $x(t)$ using finite sum
    approximations for the middle integral.

[^fn17-3]: In Examples 1 and 3 it is also possible to allow for exponential growth in the second
