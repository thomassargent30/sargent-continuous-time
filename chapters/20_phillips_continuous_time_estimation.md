...
# 20. Inferring a Continuous-Time System from Discrete-Time Data: An Appreciation of A. W. Phillips (1959)
...
written by Lars Peter Hansen and one of us.[^fn20-appreciation]
```
...
spectral density matrix**[^fn20-rational] — exactly the class of processes that Chapter 8
...
rewrites the convolution system {eq}`eq-20-sys` as a system of *differential equations* and
...
exactly what a numerator of equal or higher degree would produce in the inverse transform.[^fn20-delta]
...
Equation {eq}`eq-20-theta` deserves to be recognized for what it is: it is the **spectral
...
Combining {eq}`eq-20-Q` and {eq}`eq-20-R`, the whole system reads $Q(D)x(t) = R(D)\zeta(t)$.
...
Repeating the filtering calculation that produced {eq}`eq-20-theta`, the spectral density
...
are distinct with negative real parts, he expands {eq}`eq-20-Phi` in partial fractions and
inverts. The unique inverse compatible with stationarity — the one decaying as $\tau \to
\pm\infty$ — is

```{math}
:label: eq-20-phi-cont
...
{eq}`eq-20-phi-cont` is the centrepiece, and it is precisely the result (b) promised at the
...
function $M(z) = \sum_{\tau \ge 0} \phi_\tau z^\tau$. Substituting {eq}`eq-20-phi-cont` and
...
Equation {eq}`eq-20-log` is where the **aliasing problem** is born, and it is the same
...
factors $\mu_r$, and {eq}`eq-20-log` returns the continuous poles $\lambda_r$. The whole
...
({doc}`18_time_aggregation_var`, {doc}`21_aggregation_inverse_optimal_predictor`).
...
{eq}`eq-20-log`: P. C. B. Phillips (1972, 1973, 1974) and Hansen and Sargent (1983, 1991)
...
works.[^fn20-wheels] More recently, Hansen and Scheinkman (1995) extended the questions Phillips
...
**Aliasing and the multivalued logarithm.** Phillips's map {eq}`eq-20-log`,
...
(a one-term version of {eq}`eq-20-phi-cont` with pole $\lambda = -a$).

(a) Sample $\phi$ at the integers and form Phillips's generating function $M(z) = \sum_{\tau \ge 0}
\phi_\tau z^\tau$. Verify it equals $\phi_0/(1 - \mu z)$ with $\mu = e^{-a}$, as {eq}`eq-20-M`
predicts.

(b) Recover the continuous-time parameters from the sampled autocovariances alone:
$\mu = \phi_1/\phi_0$, then $a = -\log\mu$ from {eq}`eq-20-log`, and $b^2 = 2a\,\phi_0$.

(c) Exhibit the discrete-time shadow {eq}`eq-20-discrete`: the sampled process is the AR(1)
...
{eq}`eq-20-discrete` in one dimension. (Because the pole $\lambda = -a$ is *real*, there is no
...
(a) Sample at the integers and verify Phillips's identity {eq}`eq-20-M`: the generating function
...
(b) Recover the continuous poles from the discrete decay factors via {eq}`eq-20-log`,
$\lambda_r = \log\mu_r$, and confirm $\lambda_r = -a_r$.

```{exercise-end}
```

```{solution-start} ex-20-3
:class: dropdown
```
...
The discrete generating function reproduces Phillips's partial-fraction form {eq}`eq-20-M`
...
[^fn20-appreciation]: The framing of this chapter, the list of consequences (a)–(g) of a
...
[^fn20-rational]: For a continuous-time process, a rational spectral density is one expressible
...
[^fn20-delta]: If a numerator $\beta_{ij}(s)$ were of degree equal to or greater than its
...
[^fn20-wheels]: A manifestation of aliasing familiar to movie-goers is the illusion that a
