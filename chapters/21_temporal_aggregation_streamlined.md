...
# 21. Temporal Aggregation of Economic Time Series
...
machinery built up in the earlier chapters — the $L^2$ projection theory of Chapter 17, the
...
Chapter 17 developed the theory of linear prediction for a process observed *continuously* in
...
Marcet keeps the continuous-time framework of Chapter 17 — a random measure with orthogonal,
...
The whole analysis turns on a single idea, and it is exactly the projection idea of Chapter 17:
...
linearly regular, with finite variances. As in Chapter 17, random variables are treated as
...
$m$-dimensional orthonormal random measure. The defining isometry — the workhorse of Chapter 17
...
The proof is a direct transcription of the Chapter 17 isometry. Using
...
Chapter 17 carried out on the kernel: *the discrete forecast corresponds to approximating $a_i$
...
the continuous one. This is the kernel-level face of the aliasing of Chapter 15.
...
Chapters 15 and (in a companion paper) 22.
...
— that it assigns covariance $\delta I$ to an interval of length $\delta$ (compare Chapter 17,
{eq}`eq-17-1-1`). With $\xi^\delta$ the one-step innovation,
...
Seen alongside Chapter 17, Marcet's chapter is a cautionary complement. There, working entirely in
