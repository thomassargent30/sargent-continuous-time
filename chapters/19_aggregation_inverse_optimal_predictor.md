...
# 19. Aggregation Over Time and the Inverse Optimal Predictor Problem for Adaptive Expectations in Continuous Time
...
the 1983 original (flagged in {ref}`sec-19-5`).
```
...
problems. In the context of a continuous-time version of Cagan's portfolio schedule, they find
...
by Sims (1971, 1973). The model is simple enough that Hansen and Sargent can analyze the effect
quite completely.
...
To solve stochastic differential equation {eq}`eq-19-1`, Hansen and Sargent write it shifted
...
Projecting both sides of {eq}`eq-19-2` onto the $t$ period information set gives
...
In expression {eq}`eq-19-5` $Dp$ is not necessarily required to be an ordinary stochastic
...
realizable, the authors assume that anticipated inflation is. Equation {eq}`eq-19-5` also
...
In equation {eq}`eq-19-6` $c(D)$ is a one-sided matrix convolution operator and $w$ is a
...
where $\delta$ is the Dirac delta generalized function. They assume that {eq}`eq-19-6` holds for
some $t$ greater than or equal to a start-up time $T$, with $w(t) = 0$ for $t < T$. That
{eq}`eq-19-6` is a Wold representation means that instantaneous forecast errors in predicting an
element of $x(t)$ from past $x$'s are linear combinations of the elements of $w(t)$.

Hansen and Sargent write the first row of {eq}`eq-19-6` as
...
Shifting {eq}`eq-19-7` forward $v$ time units and taking expectations, they find
...
convolution operator that are concentrated on the negative numbers. Equation {eq}`eq-19-5`
implies that
...
Specification {eq}`eq-19-8` implies that
...
Substituting {eq}`eq-19-9` into equation {eq}`eq-19-1` shows that
...
Equation {eq}`eq-19-10` is a version of Cagan's model in continuous time, since

$$
-\frac{\beta D}{\beta + D}\, p(t) = \beta \int_{-\infty}^t e^{-\beta(t-u)} Dp(u)\, du.
$$

Equation {eq}`eq-19-10` reveals an exact relationship among $m$, $p$, and $a$. This singularity
...
Substituting {eq}`eq-19-11` into equation {eq}`eq-19-1` and equating coefficients on $w_1(t)$
...
equation {eq}`eq-19-1` empirical content, something must be said about the dynamic correlation
...
Assumption {eq}`eq-19-13` says that no other variables observed by private agents Granger cause
...
for some scalar constant $k_2$. Combining restrictions {eq}`eq-19-14` and {eq}`eq-19-12`
determines that
...
Restriction {eq}`eq-19-15` is a restriction on the bivariate moving average representation for
...
$c_{21}$, $c_{22}$. It is clear that $\beta$ and $k_1$ can be identified from {eq}`eq-19-16`. In
general, $\alpha$ and $k_2$ can be identified from equation {eq}`eq-19-15`. However, for a
...
When $a$ is a Gaussian process, {eq}`eq-19-17` implies that $a$ is in fact a Brownian motion.
Substituting {eq}`eq-19-17` into {eq}`eq-19-15` yields
...
The parameters $\alpha$ and $k_2$ are not identifiable in {eq}`eq-19-18`. It remains true,
...
For the remainder of the analysis, Hansen and Sargent adopt {eq}`eq-19-17` as a simplifying
...
In the continuous-time system {eq}`eq-19-19`, $m$ fails to Granger cause $p$ because
$c_{12}(D) = 0$, while $p$ does Granger cause $m$. That these features characterize the system
is no surprise: {eq}`eq-19-19` was constructed precisely to guarantee that Cagan's adaptive
expectations mechanism {eq}`eq-19-5` is optimal. In light of {eq}`eq-19-4`, if Cagan's mechanism
is rational there must be extensive feedback from $p$ to $m$.[^fn19-1]
...
right side of {eq}`eq-19-19` indicates that $(p, m)$ is a nonstationary process. It turns out
...
Using the Laplace transforms {eq}`eq-19-21` and {eq}`eq-19-20` gives the desired representation:
...
Then {eq}`eq-19-22` can be written
...
autocovariogram of the $y$ process is readily computed from[^fn19-2]
...
```{note}
The $(2,1)$ element of $\Gamma_1$ in {eq}`eq-19-24` is $v_{11}\tfrac{\beta}{2}\left(\tfrac{1}{3}\beta + 1\right)$.
...
integrating {eq}`eq-19-23` directly, and it is the value used in the recomputations below; it
also makes $\det[\Gamma_1' + \Gamma_0 + \Gamma_1]$ vanish, the property that the original paper
relies on (see {ref}`sec-19-5`).
```

The matrix covariogram {eq}`eq-19-24` of the discrete-time process $y_t$ contains all of the
...
process. From {eq}`eq-19-24`, $(1-L)^2 p$ is a first-order moving average with covariance
generating function
...
where from {eq}`eq-19-19`, $c(0) = 2v_{11}\left(\tfrac{1}{3}\beta^2 + 1\right)$ and
...
integers. Representation {eq}`eq-19-26` can then be written
...
{eq}`eq-19-29`, given current and lagged values of $X$ alone, is the discrete-time version of
...
Now equation {eq}`eq-19-30` is precisely the discrete-time representation which Cagan used for
...
{eq}`eq-19-28` and Cagan's formula {eq}`eq-19-31`. For $\beta$ close to zero, exp$(-\beta)$ is a
close approximation to {eq}`eq-19-28`. However, for large values of $\beta$, $\exp(-\beta)$ is
approximately zero, while equation {eq}`eq-19-28` implies a $\lambda_p$ of approximately
...
formula {eq}`eq-19-27`, if he followed Nerlove, Grether, and Carvalho's method. Using formula
{eq}`eq-19-28` or Table 1, the analyst could then infer the value of $\beta$. Table 1 provides a
fairly complete characterization of Cagan's approximation {eq}`eq-19-31` as a vehicle for
...
and $\Gamma_1$ from {eq}`eq-19-24`, $F$ and $\bar V$ are determined by solving
...
Letting $X_t = p(t) - p(t-1)$, $M_t = m(t) - m(t-1)$, one can write {eq}`eq-19-32` as
...
to verify that {eq}`eq-19-34` admits the alternative representation
...
Representation {eq}`eq-19-34` is usefully compared to the one constructed by Sargent (1977). He
...
of {eq}`eq-19-37` that Cagan's discrete-time adaptive expectations formulation for inflation is
rational, given {eq}`eq-19-37`. In form, {eq}`eq-19-37` matches {eq}`eq-19-34`. A central task is
now to study the relation between the $(2 \times 2)$ matrix $F$ in {eq}`eq-19-34` and the
...
in {eq}`eq-19-37`. Notice that the eigenvalues of the matrix $E$ are $-1$ and $-\lambda$. It can
be proved that one of the eigenvalues of $F$ in {eq}`eq-19-34` is $-1$.[^fn19-3] A comparison
between the value of $-\lambda_p$ given by equation {eq}`eq-19-28` and the nonunit eigenvalue of
...
system {eq}`eq-19-19`, $m$ fails to Granger cause $X$; however, in the discrete-time model, $M$
will in general Granger cause $X$ due to the effects of aggregation over time.[^fn19-4]
...
{eq}`eq-19-19` builds in to make Cagan's mechanism optimal. For values of $\lambda_p$ in the
...
However from {eq}`eq-19-a1` it is clear that $H_j(s, v)$ cannot depend on $v$. It follows that
...
Equation {eq}`eq-19-a4` implies that $\mu_1 = -\beta$. Therefore
...
The text states that as $\beta \to \infty$ the exact decay parameter $\lambda_p$ of {eq}`eq-19-28`
approaches $-2 + \sqrt{3}$, while Cagan's approximation $e^{-\beta}$ approaches $0$.

(a) Using {eq}`eq-19-27` with $c(0) = 2v_{11}(\tfrac{1}{3}\beta^2 + 1)$ and
...
```{solution-start} ex-19-1
:class: dropdown
```

For (a): as $\beta \to \infty$, $c(0)/c(1) = 2(\tfrac{1}{3}\beta^2 + 1)/(\tfrac{1}{6}\beta^2 - 1)
\to 2 \cdot \tfrac{1/3}{1/6} = 4$. With $r = c(0)/c(1) \to 4$, {eq}`eq-19-27` gives
...
(a) For the symmetric case $V = I$, verify numerically the claim in {ref}`sec-19-5` that **one
...
[^fn19-1]: This is a continuous-time version of the assumption about the disturbance to the
    portfolio balance schedule considered in Sargent (1977).

[^fn19-2]: Hansen and Sargent use the rules for taking expected values of products of integrals
    of white noises described by Kwakernaak and Sivan (1972, pp. 97–99).

[^fn19-3]: First, note that $(I + Fz)\bar V(I + F'z^{-1})' = \Gamma_{-1} z^{-1} + \Gamma_0 +
...
[^fn19-4]: In a more general context, Sims (1971, 1972) has emphasized that $\tilde y$'s failure
